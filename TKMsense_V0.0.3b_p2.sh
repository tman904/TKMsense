...
#Purpose:Allow beginner to configure OpenBSD as a network firewall
#Version:0.0.3b
...
maincontrol() {
            ...
            srvsinfo="Firewall=$pfstate|Routing=$iproutingstatus|DHCP=$dhcpdstate|SSH=$sshdstat"
             ...
             if [ "$tkmcfg" == "TKMsense" ] ; then
                ...
                tkmcfg="TKMsense_V0.0.3b Configured"
                autoinfo="0=Automatic Setup"

             else
                #set status to unconfigured
                tkmcfg="TKMsense_V0.0.3b Unconfigured"
             ...
             echo "2=Basic Network Settings"
             echo "3=Network Diagnostics/Troubleshooting"
             echo "4=Network Monitoring"
             echo "5=Advanced Network Settings"
             echo "#####################Menu Usage#################################################"
             ...
             if [ "$cpsetting" == "1" ] ; then
                  #Run system settings menu
                  syscom
             fi

            if [ "$cpsetting" == "2" ] ; then
                 #Run Basic network settings menu
                  basenet
            fi
            ...
            if [ "$cpsetting" == "4" ] ; then
                #run net monitor menu
                netmonit

            fi

            if [ "$cpsetting" == "5" ] ; then
                #run net advanced menu
                netadv
            fi
...
LWINT() {
      ...
      if [ "$WANCONTYPE" == "1" ] ; then
        echo "#Created by TKMsense_V0.0.3b on $LWINT_confdate" >/etc/hostname.$WANIF
      ...
      echo "#Created by TKMsense_V0.0.3b on $LWINT_confdate" >/etc/hostname.$LANIF
      ...
      #configure pf.conf
      echo "#Created by TKMsense_V0.0.3b on $LWINT_confdate" >/etc/pf.conf
      echo "" >>/etc/hostname.$WANIF
     echo "lan=\"$LANIF\"\n" >>/etc/pf.conf
     echo "wan=\"$WANIF\"\n" >>/etc/pf.conf
     echo "#dmz=\n" >> /etc/pf.conf
    ...
    #enable ip routing
    echo "#Created by TKMsense_V0.0.3b on $LWINT_confdate" >/etc/sysctl.conf
    ...
    echo "all done enjoy TKMsense"
     #return to basic networking menu
     basenet
   }
...
WTOLPF() {
  ...
  WTOLPF_confdate=`date`
        ...
        if [ "$pfans" == "2" ] ; then
            ...
            echo "" >>/etc/pf.conf
            echo "#WAN to LAN port forward created by TKMsense_V0.0.3b on $WTOLPF_confdate" >>/etc/pf.conf
            echo "" >>/etc/pf.conf
...
DHCPSERVSET() {
          ...
          echo "#Created by TKMsense_V0.0.3b on $DHCPSERVSET_confdate" >/etc/dhcpd.conf
          echo "" >>/etc/dhcpd.conf
...
autosetup() {
                ...
                if [ "$tkmcfg" == "TKMsense" ] ; then
                  ...
                  echo "Under the System Settings Menu run a factory reset then run automatic setup again."
                  sleep 4
                  #return to main menu
                  maincontrol
                          ...
                          #check current interface status
            	             if [ "$intstat" == "active" ] ; then
                               ...
                               #configure lan interface
                                 echo "#Created by TKMsense_V0.0.3b on $GUIDEME_confdate" >/etc/hostname.$aclanint
                       ...
                       do
                          ...
                          if [ "$intstat" == "active" ] ; then
                            ...
                            if [ "$WANCONTYPE" == "1" ] ; then
                              ...
                              echo "#Created by TKMsense_V0.0.3b on $GUIDEME_confdate" >/etc/hostname.$acwanint
            ...
            sleep 2
            echo "#Created by TKMsense_V0.0.3b on $GUIDEME_confdate" >/etc/sysctl.conf
            echo "" >>/etc/sysctl.conf
            echo "net.inet.ip.forwarding=1" >>/etc/sysctl.conf

            echo "configuring firewalling in /etc/pf.conf\n"
            sleep 2
            echo "#Created by TKMsense_V0.0.3b on $GUIDEME_confdate" >/etc/pf.conf
            echo "" >>/etc/pf.conf
            ...
            sleep 2
            echo "#Created by TKMsense_V0.0.3b on $GUIDEME_confdate" >/etc/dhcpd.conf
            echo "" >>/etc/dhcpd.conf
            ...
            echo "all done enjoy TKMsense."
            echo "Rebooting to apply configuration!"
            reboot
            maincontrol
        fi
}
...
pinghost() {
  ...
  if [ "$ptans" == "1" ] ; then
    ...
    sleep 5
    #return to network diagnostic menu
    netdiag
...
moncon() {
    ...
    sleep 3
    pftop
    #return to network monitoring menu
    netmonit
...
cursysstat() {
echo "How long do you want current system information to run press enter/return for default is 10 seconds"
read cursystatto
#if user doesn't set time set it to 10 secs
if [ "$cursystatto" == "" ] ; then
...
dmzint() {
  ...
  if [ ! -f /etc/pf.conf ] ; then
      ...
      sleep 8
      #return to network advanced menu
      netadv
  fi
  ...
  if [ "$curdmzint" != "" ] || [ "$dmzrlchk" != "" ] ; then
        ...
        echo "Please factory reset from the system settings menu first to assign a new one"
        sleep 6
        #return to network advanced menu
        netadv
  fi
  ...
  echo "#Created by TKMsense_V0.0.3b on $DMZINT_confdate" >/etc/hostname.$DMZIF
  ...
  if [ "$curdmzint" == "" ] ; then
    ...
    DMZINT_confdate=`date`
          ...
          if [ "$pfans" == "2" ] ; then
              ...
              #add port forwarding rule in /etc/pf.conf
              echo "" >>/etc/pf.conf
              echo "#WAN to DMZ port forward created by TKMsense_V0.0.3b on $DMZINT_confdate" >>/etc/pf.conf
              echo "" >>/etc/pf.conf
...
syscom()
{
  ...
  echo "5=Enable SSH Management"
  echo "6=Apply Security Patches"
  echo "#####################Menu Usage#################################################"
  echo "type desired option number and hit enter/return"
  echo -n "=>"
...
basenet()
{
  ...
  echo "$srvsinfo"
  echo "###############################################################################"
  echo "##############################Basic Networking Menu###########################"
  echo "0=Return To Main Menu"
  echo "1=Assign LAN and WAN Interfaces"
  echo "2=Enable/Disable DHCP Server"
  echo "3=Set LAN DHCP Server Settings"
  echo "4=Add Port Forward for WAN to LAN Traffic"
  echo "5=Enable/Disable PF Warning Also Disables NAT"
  echo "#####################Menu Usage#################################################"
  echo "type desired option number and hit enter/return"
  echo -n "=>"
...
if [ "$cpsetting" == "5" ] ; then
    #Run pf enable disable wizard
    DISPF

fi

 #user typed wrong number
 #return to basenet menu
 basenet
...
netdiag()
{
  ...
  echo "$srvsinfo"
  echo "###############################################################################"
  echo "##############################Network Diagnostic Menu##########################"
  echo "0=Return to Main Menu"
  echo "1=Ping Host"
  echo "2=Traceroute to Host"
  echo "3=WAN Speedtest"
  echo "4=Show ARP Cache"
...
sysupgrd()
{
echo "Welcome to system patching wizard.\n"
echo "Do you want to proceed to check for and apply patches?\n"
echo "1=yes"
echo "2=no"
read ans

	if [ "$ans" == "1" ] ; then
		#run syspatch and reboot
		echo "Applying patches and if needed rebooting"
		syspatch
		reboot
	fi
...
netmonit()
{
  ...
  echo "2=UDP DNS Query Monitoring"
  echo "3=Bandwidth Usage"
  echo "4=Firewall Status"
  echo "5=Firewall Resources"
  echo "6=Packet Monitoring"
  echo "#####################Menu Usage#################################################"
  echo "type desired option number and hit enter/return"
  echo -n "=>"
...
netadv()
{
  ...
  echo "$srvsinfo"
  echo "###############################################################################"
  echo "################################Advanced Networking Menu#######################"
  echo "0=Return to Main Menu"
  echo "1=Assign DMZ Interface"
  echo "2=Add Port Forward for WAN to DMZ Traffic"
  echo "#####################Menu Usage#################################################"
  echo "type desired option number and hit enter/return"
  echo -n "=>"
